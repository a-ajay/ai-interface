...
import { PromptContext, OrchestratorConfig, EnhancedPrompt } from '../src/types';
...
class IntegrationTests {
  ...
  private async testTypicalUserWorkflow(): Promise<void> {
  ...
  private async testDifferentProgrammingLanguages(): Promise<void> {
  ...
  private async testComplexCodeContexts(): Promise<void> {
  ...
  private async testVariousPromptTypes(): Promise<void> {
  ...
  private async testConfigurationScenarios(): Promise<void> {
    ...
    const configs = [
      { name: 'Conservative (Light)', compressionLevel: 'light' as const, tokenSavingsTarget: 15 },
      { name: 'Balanced (Moderate)', compressionLevel: 'moderate' as const, tokenSavingsTarget: 30 },
      { name: 'Aggressive', compressionLevel: 'aggressive' as const, tokenSavingsTarget: 50 }
    ];

    for (const config of configs) {
      const enhancerConfig: OrchestratorConfig = {
        ...testConfig,
        compressionLevel: config.compressionLevel,
        tokenSavingsTarget: config.tokenSavingsTarget
      };

      const enhancer = new EnhancerCore(enhancerConfig);
      const optimizer = new TokenOptimizer(enhancerConfig);
