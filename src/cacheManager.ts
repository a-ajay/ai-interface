...
export class CacheManager {
   ...
   */
  public put(prompt: string, response: string, metadata?: CacheEntry['metadata']): void {
   ...
   */
  public findSimilar(prompt: string): { entry: CacheEntry; similarity: number } | null {
    const normalizedPrompt = this.normalizePrompt(prompt);
    const similarEntry = this.findSimilarEntry(normalizedPrompt);
   ...
   */
  private findSimilarEntry(normalizedPrompt: string): CacheEntry | null {
   ...
   */
  public getEntryDetails(prompt: string): CacheEntry | null {
    const normalizedPrompt = this.normalizePrompt(prompt);
    const hash = this.hashPrompt(normalizedPrompt);
